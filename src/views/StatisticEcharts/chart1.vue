    ...
    <div>
        ...
        <el-row :gutter="20" style="margin-top: 10px;">
            <el-col :span="20" :offset="1">
                ...
                <div id="chartContainer1" class="chartContainer1" style="width: 100%;"></div>
        ...
        <el-row :gutter="20" style="margin-top: 10px;">
            <el-col :span="20" :offset="1">
                <div id="chartContainer2" class="chartContainer2" style="width: 100%;"></div>
            </el-col>
        </el-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //查询统计数据
            queryByItem() {
                if (null === this.deptType || this.deptType.length <= 0) {
                    this.$message.error("请选择部门！");
                } else if (null === this.value || this.value.length <= 0) {
                    this.$message.error("请选择统计项目！");
                } else if (null === this.value2 || this.value2.length <= 0) {
                    this.$message.error("请选择时间段！");
                } else if (null === this.value3 || this.value3.length <= 0) {
            ...
            chart1Fun() {
                ...
                for (var i = 0; i < this.dept.length; i++) {
                    if (this.currValue[i] === undefined || this.currValue[i].trim() === "-") {
                        this.currValue[i] = "0";
                    }
                    if (this.growthList[i] === undefined || this.growthList[i].trim() === "-") {
                        this.growthList[i] = "0";
                    }
                    if (this.growthRateList[i] === undefined || this.growthRateList[i].trim() === "-") {
                ...
                // Themes end
                am4core.options.queue = true;//多个图表排队显示
                //am4core.options.onlyShowOnViewport = true;//延迟加载
                //创建图例
                var chart = am4core.create("chartContainer1", am4charts.XYChart3D);
                ...
                chart.scrollbarX = new am4core.Scrollbar();//滚动条
                chart.responsive.enabled = true;
                chart.exporting.menu = new am4core.ExportMenu();//导出图表
                chart.exporting.filePrefix = formatDate(this.value3, "YYYY") + "年" + this.value2Name + this.valueName;
                ...
                /************第二个图************/
                    //创建图例
                var chart2 = am4core.create("chartContainer2", am4charts.XYChart3D);

                chart2.data = data2;
                // Create axes
                let categoryAxis2 = chart2.xAxes.push(new am4charts.CategoryAxis());
                categoryAxis2.dataFields.category = "dept";
                categoryAxis2.renderer.labels.template.rotation = 315;
                ...
                chart2.cursor = new am4charts.XYCursor();//坐标轴定位
                chart2.scrollbarX = new am4core.Scrollbar();//滚动条
                chart2.exporting.menu = new am4core.ExportMenu();//导出图表
                chart2.exporting.filePrefix = formatDate(this.value3, "YYYY") + "年" + this.value2Name + this.valueName;

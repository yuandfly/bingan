...
<script>
import { serachDepartmentPhysician } from "@/api";
  ...
  methods: {
    ...
    getTableList() {
      if (this.dateList.dateBegin && this.dateList.dateEnd) {
        serachDepartmentPhysician({
          bz: 61,
          kb: "0000",
          qs: this.dateList.dateBegin,
          rls: 0,
          zz: this.dateList.dateEnd
        }).then(res => {
          if (res.code === 0) {

...
<template @keyup.113.native="created">
	<el-dialog title="代码录入窗口" :visible.sync="visible" @close="$emit('update:show1', false)" :show="show1" width="20%" :modal="false"
	 @opened="AUTO()" append-to-body>
...
<script>
	...
	export default {
		...
		methods: {
			...
			dialog(row) {
				this.$parent.fatherMethod(row, this.msgs,1);
			},
			/**input框获取焦点*/
			AUTO() {

...
<template slot-scope="props">
    <section>
        ...
        <OperationNameBox ref="child"  @fun="queryAll"></OperationNameBox>
    </section>
</template>
...
<script>
    ...
    import OperationNameBox from './OperationNameBox';
    export default {
        components: {
            OperationNameBox,
        },
        ...
        methods: {
            ...
            queryAll(condition) {
                ...
                GetOperationName(this.form).then(res => {
                    if (res.data.code === 0) {
            ...
            handleDelete(row) {
                this.$confirm("确认删除吗?", "提示", {
                    ...
                    .then(() => {
                        DeleteOperationName(row).then((res) => {
                            console.log(res);
                            if (res.data.code === 0) {
                                this.$message({
                                    type: "success",
            ...
            update(row) {
                this.$confirm("确定要修改此项吗, 是否继续?", "提示", {
                    ...
                    .then(() => {
                        ...
                        UpdateOperationName(row).then((res) => {
                            if (res.data.code === 0) {
                                this.$message({
                                    type: "success",
                                    message: "修改成功!",

...
<template slot-scope="props">
    <section>
        <el-form :inline="true" :model="form" class="demo-form-inline" size="medium"  >
            <div style="padding: 20px;">
                ...
                >
                    ...
                    <el-table-column prop="dmmc" label="单位、地址名称">
                        <template slot-scope="scope">
                            <el-input v-model.trim="scope.row.dmmc" :disabled="scope.row.bz === '1' || scope.row.bz === null"  size="small" placeholder="请输入"></el-input>
                        </template>
                    </el-table-column>
                    <el-table-column prop="yzbm" label="邮编">
                        <template slot-scope="scope">
                            <el-input v-model.trim="scope.row.yzbm" :disabled="scope.row.bz === '1' || scope.row.bz === null"  size="small" placeholder="请输入"></el-input>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="操作" width="220">
                        <template slot-scope="scope">
                            <el-button size="mini" @click="update(scope.row, scope.$index)" type="primary">修改</el-button>
        ...
        <UnitAddressBox ref="child"  @fun="queryAll"></UnitAddressBox>
    </section>
</template>
...
<script>
    ...
    import UnitAddressBox from './UnitAddressBox';
    export default {
        components: {
            UnitAddressBox,
        },
        ...
        methods: {
            ...
            queryAll(condition) {
                ...
                GetUnitAddress(this.form).then(res => {
                    if (res.data.code === 0) {
                        if (res.data.data.content) {
                            this.tableData = res.data.data.content;
                            this.tableDataCopy = JSON.parse(JSON.stringify(res.data.data.content));
            ...
            handleDelete(row) {
                this.$confirm("确认删除吗?", "提示", {
                    ...
                    .then(() => {
                        DeleteUnitAddress(row).then((res) => {
                            console.log(res);
                            if (res.data.code === 0) {
                                this.$message({
                                    type: "success",
                ...
                this.$confirm("确定要修改此项吗, 是否继续?", "提示", {
                    ...
                    .then(() => {
                        ...
                        let params = JSON.parse(JSON.stringify(row));
                            params.newDmmc = row.dmmc;
                            params.dmmc = this.tableDataCopy[index].dmmc;
                        console.log(params);
                        UpdateUnitAddress(params).then((res) => {
                            if (res.data.code === 0) {
                                this.$message({
                                    type: "success",
                                    message: "修改成功!",

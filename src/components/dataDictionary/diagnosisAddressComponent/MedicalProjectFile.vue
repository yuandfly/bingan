...
<template slot-scope="props">
    <section>
        <el-form :inline="true" :model="form" class="demo-form-inline" size="medium"  >
            <div style="padding: 20px;">
                <el-table
                        ...
                        border
                >
                    <el-table-column prop="dm" label="归口代码">
                    </el-table-column>
                    <el-table-column prop="dmmc" label="归口名称">
                    ...
                    <el-table-column prop="dw" label="单位">
                        <template slot-scope="scope">
                            <el-input v-model.trim="scope.row.dw" size="small" :disabled="scope.row.bz === '1' || scope.row.bz === null"  placeholder="请输入"></el-input>
                        </template>
                    </el-table-column>
        ...
        <MedicalProjectFileBox ref="child"  @fun="queryAll"></MedicalProjectFileBox>
    </section>
</template>
...
<script>
    import {
        ...
        DeleteMedicalProjectFile,
        UpdateMedicalProjectFile,
    } from "@/api";
    import MedicalProjectFileBox from './MedicalProjectFileBox';
    export default {
        components: {
            MedicalProjectFileBox,
        },
        data() {
            return {
                pageSizes: 10,
        ...
        methods: {
            ...
            queryAll(condition) {
                ...
                GetMedicalProjectFile(this.form).then(res => {
                    if (res.data.code === 0) {
            ...
            handleDelete(row) {
                this.$confirm("确认删除吗?", "提示", {
                    ...
                    .then(() => {
                        DeleteMedicalProjectFile(row).then((res) => {
                            console.log(res);
                            if (res.data.code === 0) {
                                this.$message({
                                    type: "success",
            ...
            update(row) {
                this.$confirm("确定要修改此项吗, 是否继续?", "提示", {
                    ...
                    .then(() => {
                        ...
                        UpdateMedicalProjectFile(row).then((res) => {
                            if (res.data.code === 0) {
                                this.$message({
                                    type: "success",
                                    message: "修改成功!",
